...
public interface DeleteClientById extends JpaRepository<Archive, Integer> {
    ...
    @Modifying
    @Query(value = """ 
        DELETE FROM meeting_agency.characters_list WHERE characters_list.User_ID = ?1
""", nativeQuery = true)
    ...
    @Modifying
    @Query(value = """ 
        DELETE FROM meeting_agency.requirements_list WHERE requirements_list.User_ID = ?1
""", nativeQuery = true)
    ...
    @Modifying
    @Query(value = """ 
        DELETE FROM meeting_agency.hobbylist WHERE hobbylist.User_ID = ?1
""", nativeQuery = true)
    ...
    @Modifying
    @Query(value = """ 
        DELETE FROM meeting_agency.client WHERE client.ID = ?1
""", nativeQuery = true)
